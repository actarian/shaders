...
void main() {
    ...
    if (between(0.5, 0.25)) {
        v = easeElasticOut(animation.pow);
        object.distance = plot(st, 0.5 * v);
    }

    if (between(0.25)) {
        v = easeSineInOut(animation.pow);
        object.distance = plot(st, 0.5, PI_TWO / 2.0 * v);
    }

    if (between(0.5)) {
        v = easeBounceOut(animation.pow);
        object.distance = plot(st, 0.5 * (1.0 - v), PI_TWO / 2.0);
    }

    if (between(1.0, 0.25)) {
        v = easeQuintOut(animation.pow);
        v2 = easeQuintIn(animation.pow);
        object.distance = line(st + vec2(mix(-0.5, 0.5, v), 0.0), st + vec2(mix(-0.5, 0.5, v2), 0.0), 0.004);
    }
    if (between(1.0, -0.8)) {
        v = easeQuintOut(animation.pow);
        v2 = easeQuintIn(animation.pow);
    ...
    if (between(1.0, 0.25)) {
        v = easeBounceOut(animation.pow);
        object.distance = circle(st + vec2(0.0, mix(-0.3, 0.3, v)), 0.2, 0.02);
    }

    if (between(0.5)) {
        v = easeSineOut(animation.pow);
        object.distance = circle(st + vec2(0.3 * cos(PI_TWO + v * 2.0 * PI), 0.3 * sin(PI_TWO + v * 2.0 * PI)), 0.2, 0.02);
    }

    if (between(1.0)) {
        v = easeElasticOut(animation.pow);
        object.distance = circle(st + vec2(0.0, mix(0.3, 0.0, v)), 0.2 + 0.4 * v, 0.02 + 0.06 * v);
    }

    if (between(0.15)) {
        v = easeSineOut(animation.pow);
        object.distance = circle(st, 0.6 - 0.5 * v, 0.08) * (1.0 - v);
    }

    if (between(0.5)) {
        v = easeElasticOut(animation.pow);
        object.distance = poly(st * rotate2d(PI), 0.1 + 0.2 * v, 3, 0.06);
    }

    if (between(0.35)) {
        v = easeCircularOut(animation.pow);
        object.distance = poly(st * rotate2d(PI) + vec2(0.0, mix(0.6, 0.0, v)), 0.1, 3, 0.02);
    }
    if (between(0.35)) {
        v = easeCircularIn(animation.pow);
        object.distance = poly(st * rotate2d(PI) + vec2(0.0, mix(0.0, -0.6, v)), 0.1, 3, 0.02);
    }

    if (between(1.0, 0.25)) {
        v = easeQuintOut(animation.pow);
        v2 = easeQuintIn(animation.pow);
        object.distance = line(st + vec2(0.0, mix(-0.5, 0.5, v)), st + vec2(0.0, mix(-0.5, 0.5, v2)), 0.004);
    }
    if (between(1.0, -0.8)) {
        v = easeQuintOut(animation.pow);
        v2 = easeQuintIn(animation.pow);
    ...
    if (between(1.0)) {
        v = easeCircularIn(animation.pow);
        object.distance = star(st, 0.5, 5 + int(animation.pow * 15.0), 0.04);
    }

    if (between(0.5)) {
        v = easeCircularIn(animation.pow);
        object.distance = star(st + vec2(0.0, mix(0.0, 0.5, v)), 0.5, 20, 0.04) * (1.0 - animation.pow);
    }
