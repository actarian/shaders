...
float stroke(in float d, in float size, in float t) { return 1.0 - smoothstep(t - rx, t + rx, abs(d - size)); }
...
float sArc(in vec2 p, in float s, in float e, in float size) {
	...
	float d = distance(p, size * 0.5 * vec2(cos(a), sin(a))) + size;
    return d;
}
