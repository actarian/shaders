...
void animate(in float diff) {
    ...
    if (between(2.0)) {
        ...
        object.distance = circle(p, pix(50.0), pix(2.0));
    }
    ...
    if (between(6.0)) {
        ...
        object.distance = roundrect(p, pix(50.0, 50.0), pix(5.0), pix(2.0));
    }
    ...
    if (between(2.0)) {
        ...
        object.distance = poly(p, pix(50.0), 4, pix(2.0));
    }
...
void main() {
    ...
    color = mix(color, MAGENTA, 
        roundrect((pos(-125.0, -125.0)) * rotate2d(-u_time), pix(100.0, 100.0), pix(10.0))
    );

    color = mix(color, MAGENTA, 
        roundrect((pos(125.0, -125.0)) * rotate2d(u_time), pix(100.0, 100.0), pix(10.0), pix(1.0))
    );

    color = mix(color, MAGENTA, 
        circle(pos(-125.0, 125.0), pix(30.0))
    );

    color = mix(color, MAGENTA, 
        circle(pos(125.0, 125.0), pix(30.0), pix(1.0))
    );
  
    color = mix(color, MAGENTA, 
        rect(pos(-125.0, 225.0), pix(90.0 + sin(u_time) * 60.0, 90.0 + cos(u_time) * 60.0))
    );

    color = mix(color, MAGENTA, 
        rect(pos(125.0, 225.0), pix(90.0 + cos(u_time) * 60.0, 90.0 + sin(u_time) * 60.0), pix(1.0))
    );

    color = mix(color, YELLOW, 
        poly(pos(-125.0, -225.0), pix(30.0), 3)
    );

    color = mix(color, YELLOW, 
        poly(pos(125.0, -225.0), pix(30.0), 3, pix(4.0))
    );
...
float _line(vec2 a, vec2 b, vec2 p, float size) {
    ...
    return smoothstep(size + rx, size - rx, f);
}
